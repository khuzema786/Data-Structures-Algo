...
{
    if (s[u] < s[v]) // whichever has more number of children, that will continue to remain as parent
    {
        s[u] += s[v];
        s[v] = u;
    }
...
int Find(int u, int s[])
{
    ...
    // Connected all vertices directly to parent
    // while (u != x)
    // {
    //     v = s[u];
    //     s[u] = x;
