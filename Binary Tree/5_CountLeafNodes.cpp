...
public:
    ...
    int leafNodeCount(Node *p); // deg 0
...
int Tree::leafNodeCount(Node *p)
{
    ...
    if (p != nullptr)
    {
        ...
        if (p->lchild == nullptr && p->rchild == nullptr) // If no child then increment 1
        {
            return x + y + 1;
        }
        else
...
int Tree::deg1NodeCount(Node *p)
{
    ...
    if (p != nullptr)
    {
        ...
        if (p->lchild != nullptr ^ p->rchild != nullptr) // (p->lchild == nullptr && p->rchild != nullptr) || (p->lchild != nullptr && p->rchild == nullptr) => A EXOR B = A^B
        {
            return x + y + 1;
        }
        else
